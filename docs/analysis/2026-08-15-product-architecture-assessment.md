...
| **Entitlement/serviço concedido**                                 | **MVP (mínimo)**           | `subscription.active` precisa virar uma resposta consultável — senão cada produto reimplementa "esse cliente pode usar?". Uma rota `GET /v1/entitlements?customer_ref=` resolve. |
