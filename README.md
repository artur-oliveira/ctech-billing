...
| `api/internal/email` | The SES client for dunning reminders, and the one template. Small on purpose — notification delivery is barely billing's domain, and the family's convention is that a service sends what it is responsible for saying. |
...
|---|---|
| `GET /v1/health` | — |
| `POST /v1/customers` · `GET /v1/customers/:id` | `billing:customers:write` / `:read` |
| `POST /v1/subscriptions` · `GET /v1/subscriptions/:id` · `POST /v1/subscriptions/:id/cancel` | `billing:subscriptions:write` / `:read` |
| `POST /v1/usage` | `billing:usage:write` |
| `GET /v1/invoices` · `GET /v1/invoices/:id` | `billing:invoices:read` |
| `GET /v1/entitlements?customer_ref=` | `billing:entitlements:read` |
...
`POST /v1/usage` therefore carries a `price_id` saying which item the consumption
...
| Route | Scope |
|---|---|
| `GET /v1/console/session` | `billing:organization:read` |
| `GET /v1/console/invoices?year=&month=&cursor=` · `GET /v1/console/invoices/:id` | `billing:invoices:read` |
| `GET /v1/console/subscriptions?cursor=` · `GET /v1/console/subscriptions/:id` | `billing:subscriptions:read` |
| `GET /v1/console/customers?cursor=` · `GET /v1/console/customers/:id` | `billing:customers:read` |
| `GET /v1/console/products` · `GET /v1/console/products/:id` | `billing:products:read` |
| `POST /v1/console/subscriptions/:id/cancel` | `billing:subscriptions:write` |
...
| Route | Scope |
|---|---|
| `GET /v1/me` | — (identity only; says which shells this person can open) |
| `GET /v1/portal/session` · `/portal/subscriptions` · `/portal/subscriptions/:id` | `billing:me:subscriptions:read` |
| `GET /v1/portal/invoices` · `/portal/invoices/:id` | `billing:me:invoices:read` |
| `POST /v1/portal/invoices/:id/pay` | `billing:me:invoices:write` |
| `POST /v1/portal/subscriptions/:id/cancel` | `billing:me:subscriptions:write` |
...
|---|---|
| `GET /v1/checkout/:token` · `POST /v1/checkout/:token/pay` | the signed link token itself |
| `POST /v1/internal/webhooks/wallet` | wallet's `X-Wallet-Signature` HMAC |
...
| `/maintenance` · `/404` | Whole-page states. A 503 from any request sends the reader here with the path they came from, and the page probes `/v1/health` and carries them back. |
