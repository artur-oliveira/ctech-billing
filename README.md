...
| `ui/` | The Next 16 portal, built and passing: the public landing, the OAuth round trip, the four signed-in screens (P1–P4), and the public checkout (X1). A production build is `output: "export"` — a directory of files, no server ([ADR 0013](docs/adr/0013-static-portal-same-origin-api.md)), deployed to Cloudflare Workers Static Assets ([ADR 0020](docs/adr/0020-portal-on-cloudflare-workers.md)) by `ctech-cdk`'s reusable workflow. Primitives come from `@aoctech/ui` on npm, built in the sibling repo `ctech-ui`; `DESIGN.md` records the token system and what was rejected, `PRODUCT.md` the users and the screen list. |
