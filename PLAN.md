...
- [x] Contract tests against a real signed-token issuer (an httptest JWKS server), not a
      bypassed verifier.
- [x] Console read API (`/v1/console/*`) — the backend the console screens C1–C9 and C17 need:
...
- [x] Portal read API (`/v1/portal/*`) plus `GET /v1/me` — the consumer surface for **CTech's own
...
- [x] **Subscription cancellation, on both surfaces.** Immediate and at-period-end are two
    ...
    - [x] Console: `POST /v1/console/subscriptions/:id/cancel`, both modes, actor is the signed-in
    ...
    - [x] Portal: `POST /v1/portal/subscriptions/:id/cancel`, scope
...
- [x] UI foundation + portal P1–P3. Next 16 app in `ui/`, two route shells in one app, on
    ...
    - [x] **Static export, and the rule that follows from it.** A production build is
          ...
          are no dynamic route segments: an invoice is `/invoice?id=…`. `/v1/*` is forwarded by the
...
- [x] **Recent invoices on the subscription detail.** One attribute on the invoice row, reusing the
      ...
      what one screen asks. Returned only from `GET /v1/portal/subscriptions/:id`.
...
- [x] Wallet charge client (`internal/wallet`) + `POST /v1/internal/webhooks/wallet`. The webhook
...
- [x] **X1 — the hosted checkout**, in both halves: `POST /v1/portal/invoices/:id/pay` for a
  signed-in customer, and `GET|POST /v1/checkout/:token` for a payment link with no session at
...
1. **Configuration in other repositories.** Nothing here can proceed without it and none of it is
   code in this repo — Phase 0's last item, and the table in
...
3. **`@aoctech/ui` is unpublished.** `ui/` consumes it through a `file:` link to a sibling checkout,
   which is why `next.config.ts` widens the Turbopack root. CI cannot build the front end until the
   package exists on npm. Narrow the root back in the same change.
4. **A verified SES identity** and `EMAIL_FROM` set to it, or dunning will not start. The IAM
   policy pins the sender address, so the Terraform variable and the SSM value have to agree.
5. **Console writes, then the console** (Phase 2). In that order, for the reason each item gives.
...
1. ~~Wallet charge lifecycle~~ — resolved as scope
   ([ADR 0004](docs/adr/0004-pix-on-invoice-via-wallet.md)) and now as a written contract
   ([spec](docs/specs/2026-08-15-wallet-invoice-charge.md)), verified line by line against wallet's
   real source rather than against its docs. **Still the one cross-repo dependency and the only
   item that does not depend on this repository alone** — what remains is implementing it in
   `ctech-wallet`.
