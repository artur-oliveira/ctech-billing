...
func TestPortalResolvesTheCustomerFromTheAccount(t *testing.T) {
	...
	res := e.console(t, "/v1/portal/session", token, "")
...
func TestPortalRejectsANonCustomer(t *testing.T) {
	...
	res := e.console(t, "/v1/portal/session", stranger, "")
...
func TestServiceTokensAreRejectedOnPortalRoutes(t *testing.T) {
	...
	res := e.console(t, "/v1/portal/session", m2m, "")
	if res.status != http.StatusForbidden {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
}
...
func TestPortalAndConsoleScopesDoNotOverlap(t *testing.T) {
	...
	if res := e.console(t, "/v1/portal/invoices", consoleScoped, ""); res.status != http.StatusForbidden {
	...
	if res := e.console(t, "/v1/console/invoices", portalScoped, "live"); res.status != http.StatusForbidden {
...
func TestPortalShowsOnlyTheSignedInCustomersInvoices(t *testing.T) {
	...
	res := e.console(t, "/v1/portal/invoices", token, "")
	if res.status != http.StatusOK {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
	var page struct {
...
func TestPortalHidesAnotherCustomersInvoice(t *testing.T) {
	...
	res := e.console(t, "/v1/portal/invoices/"+theirs.ID, token, "")
	if res.status != http.StatusNotFound {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
}
...
func TestPortalPayloadCarriesNoInternalVocabulary(t *testing.T) {
	...
	res := e.console(t, "/v1/portal/invoices/"+inv.ID, token, "")
	if res.status != http.StatusOK {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
...
// /v1/me answers which shells this person can open, without either resolver.
func TestMeReportsBothIdentities(t *testing.T) {
	...
	res := e.console(t, "/v1/me", consumer, "")
	if res.status != http.StatusOK {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
	var body struct {
	...
	res = e.console(t, "/v1/me", operator, "")
	res.decode(t, &body)
