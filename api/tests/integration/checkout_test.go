...
func (e *payEnv) openCharge(t *testing.T, inv *billing.Invoice) string {
	...
	e.post(t, "/v1/portal/invoices/"+inv.ID+"/pay", token, "").decode(t, &body)
...
func (e *payEnv) notify(t *testing.T, chargeID string) apiResponse {
	...
	req := httptest.NewRequest(http.MethodPost, "/v1/internal/webhooks/wallet", strings.NewReader(body))
...
func TestPayingAnInvoiceEndToEnd(t *testing.T) {
	...
	res := e.post(t, "/v1/portal/invoices/"+inv.ID+"/pay", token, "")
...
func TestPayingTwiceReusesTheSameCharge(t *testing.T) {
	...
	e.post(t, "/v1/portal/invoices/"+inv.ID+"/pay", token, "").decode(t, &first)
	e.post(t, "/v1/portal/invoices/"+inv.ID+"/pay", token, "").decode(t, &second)
...
func TestWebhookRejectsABadSignature(t *testing.T) {
	e := newPayEnv(t)
	req := httptest.NewRequest(http.MethodPost, "/v1/internal/webhooks/wallet",
...
func TestPaymentLinkOpensOneInvoiceWithoutSigningIn(t *testing.T) {
	...
	if res := e.post(t, "/v1/checkout/"+link+"/pay", "", ""); res.status != http.StatusOK {
		t.Fatalf("pay: %d %s", res.status, res.body)
	}
	if e.wallet.opens != 1 {
		t.Fatalf("wallet opened %d charges, want 1", e.wallet.opens)
...
func TestPaymentLinkRejectsAForgedToken(t *testing.T) {
	...
	res := e.do(t, http.MethodGet, "/v1/checkout/"+forged, "", "", "")
...
func TestPaymentLinkHidesADraftInvoice(t *testing.T) {
	...
	if res := e.do(t, http.MethodGet, "/v1/checkout/"+link, "", "", ""); res.status != http.StatusNotFound {
		t.Fatalf("status = %d, want 404: %s", res.status, res.body)
	}
}
...
func TestChargesAreRefusedWithoutAPayerAccount(t *testing.T) {
	...
	link := e.links.Sign(e.org.ID, true, inv.ID)
	res := e.post(t, "/v1/checkout/"+link+"/pay", "", "")
	if res.status != http.StatusConflict {
		t.Fatalf("status = %d, want 409: %s", res.status, res.body)
	}
	if e.wallet.opens != 0 {
...
func TestPortalCancellationIsAtPeriodEnd(t *testing.T) {
	...
	res := e.post(t, "/v1/portal/subscriptions/"+sub.ID+"/cancel", token, "")
	if res.status != http.StatusOK {
	...
	if res := e.post(t, "/v1/portal/subscriptions/"+sub.ID+"/cancel", token, ""); res.status != http.StatusOK {
...
func TestPortalCancellationNeedsTheWriteScope(t *testing.T) {
	...
	if res := e.post(t, "/v1/portal/subscriptions/"+sub.ID+"/cancel", readOnly, ""); res.status != http.StatusForbidden {
...
func TestConsoleCancellationIsAttributedToTheOperator(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost,
		"/v1/console/subscriptions/"+sub.ID+"/cancel", strings.NewReader(`{"at_period_end":false}`))
	req.Header.Set("Content-Type", "application/json")
