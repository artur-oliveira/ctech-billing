...
func TestHealthNeedsNoToken(t *testing.T) {
	e := newAPI(t)
	res := e.do(t, http.MethodGet, "/v1/health", "", "", "")
...
func TestUnauthenticatedRequestsAreRejected(t *testing.T) {
	...
	for _, tc := range []struct{ name, token string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			res := e.do(t, http.MethodGet, "/v1/invoices", tc.token, "", "")
...
func TestScopeIsEnforcedPerRoute(t *testing.T) {
	...
	res := e.do(t, http.MethodPost, "/v1/subscriptions", readOnly, "k1", `{"customer_id":"x","price_id":"y"}`)
...
func TestUserTokensAreRejectedOnM2MRoutes(t *testing.T) {
	...
	res := e.do(t, http.MethodPost, "/v1/customers", userToken, "k1", `{"name":"Fulana"}`)
	if res.status != http.StatusForbidden {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
}
...
func TestUnknownClientIsRejected(t *testing.T) {
	...
	res := e.do(t, http.MethodPost, "/v1/customers", stranger, "k1", `{"name":"Fulana"}`)
	if res.status != http.StatusForbidden {
		t.Fatalf("status = %d: %s", res.status, res.body)
	}
}
...
func TestSubscribeThroughTheAPI(t *testing.T) {
	...
	res := e.do(t, http.MethodPost, "/v1/customers", writeToken, "cust-1",
	...
	res = e.do(t, http.MethodPost, "/v1/subscriptions", writeToken, "sub-1", body)
	if res.status != http.StatusCreated {
	...
	res = e.do(t, http.MethodGet, "/v1/invoices/"+created.Invoice.ID, writeToken, "", "")
	if res.status != http.StatusOK {
	...
	res = e.do(t, http.MethodGet, "/v1/entitlements?customer_ref=user_42", writeToken, "", "")
	if res.status != http.StatusOK {
	...
	res = e.do(t, http.MethodPost, "/v1/subscriptions/"+created.Subscription.ID+"/cancel",
...
func TestIdempotencyReplaysTheFirstResponse(t *testing.T) {
	...
	first := e.do(t, http.MethodPost, "/v1/customers", token, "retry-me", body)
	if first.status != http.StatusCreated {
		t.Fatalf("first: %d %s", first.status, first.body)
	}
	second := e.do(t, http.MethodPost, "/v1/customers", token, "retry-me", body)
	...
	conflict := e.do(t, http.MethodPost, "/v1/customers", token, "retry-me", `{"name":"Outra Pessoa"}`)
...
func TestMutatingRoutesRequireAnIdempotencyKey(t *testing.T) {
	...
	res := e.do(t, http.MethodPost, "/v1/customers", token, "", `{"name":"Fulana"}`)
...
func TestCredentialsCannotReachAnotherTenant(t *testing.T) {
	...
	res := e.do(t, http.MethodGet, "/v1/customers/"+stranger.ID, token, "", "")
