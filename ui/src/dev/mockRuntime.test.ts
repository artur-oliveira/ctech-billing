...
describe("scenario selection", () => {
  ...
  it("promotes ?scenario= to storage so client navigation does not lose it", async () => {
    ...
    await get<ListResponse<Invoice>>("/v1.0/portal/invoices")
...
describe("paying", () => {
  it("returns a PIX charge that has not already expired", async () => {
    ...
    const {data} = await post<PaymentResult>("/v1.0/portal/invoices/inv_mock_0042/pay")
  ...
  it("refuses an invoice the server would refuse", async () => {
    setScenario("paga")
    await expect(post("/v1.0/portal/invoices/inv_mock_0042/pay")).rejects.toMatchObject({
      response: {status: 422},
    })
  })
...
describe("failure fixtures", () => {
  it("fails like a lost connection, not like a server error", async () => {
    ...
    await expect(get("/v1.0/portal/invoices")).rejects.toMatchObject({code: "ERR_NETWORK"})
  })

  it("404s a route it has no fixture for, instead of answering with nothing", async () => {
    setScenario("em_dia")
    await expect(get("/v1.0/portal/nao_existe")).rejects.toMatchObject({
      response: {status: 404},
    })
  })
})
