...
export const mockAdapter: AxiosAdapter = async config => {
  ...
  if (url.endsWith("/v1/health")) return ok(config, {status: "ok"})

  if (url.endsWith("/v1/portal/session")) return ok(config, MOCK_CUSTOMER)

  if (url.endsWith("/v1/portal/subscriptions")) {
    return ok(config, {data: fixture().subscriptions, has_more: false})
  }

  const cancel = url.match(/\/v1\/portal\/subscriptions\/([^/]+)\/cancel$/)
  ...
  if (url.endsWith("/v1/portal/invoices")) {
    return ok(config, {data: invoices(), has_more: false})
  }

  const pay = url.match(/\/v1\/portal\/invoices\/([^/]+)\/pay$/)
  if (pay && method === "post") {
  ...
  const checkoutPay = url.match(/\/v1\/checkout\/([^/]+)\/pay$/)
  ...
  const checkout = url.match(/\/v1\/checkout\/([^/]+)$/)
  if (checkout) {
  ...
  const detail = url.match(/\/v1\/portal\/invoices\/([^/]+)$/)
  if (detail) {
