...
|---|---|---|
| `NEXT_PUBLIC_API_URL` | **empty** | CloudFront forwards `/v1/*` to the API from the same distribution, so the browser is same-origin and CORS never applies. |
...
| `src/app/{not-found,error}.tsx`, `src/app/maintenance/` | The whole-page states. Outside the group on purpose — all three mean the nav is useless or misleading. A 503 from any request sends the reader to `/maintenance`, which probes `/v1/health` every 15s and carries them back. |
...
"Assinaturas". The URL is part of the API surface and matches `/v1/portal/invoices`; the text on
