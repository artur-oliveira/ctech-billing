...
**Credentials come from the standard AWS chain — export `AWS_PROFILE` yourself.**
Neither root names a profile, in the backend or on the provider, and that is
deliberate: a backend block cannot take a variable, so a hardcoded
`profile = "ctech"` forced CI to fake a profile that referred to credentials it
already had. It faked it wrongly (`credential_source` without `role_arn` is not
a valid profile) and `terraform init` refused every deploy. In CI the assumed
role's environment variables are found with no configuration at all.
...
If you have an existing `.terraform/` from before the profile was removed,
Terraform will report the backend configuration changed. `terraform init
-reconfigure` is the whole fix — the state itself has not moved, only how
Terraform authenticates to reach it.
...
The distribution has **two origins**. `/v1.0/*` and `/.well-known/*` are ordered cache behaviours
...
to `/404.html` would also replace the API's RFC 7807 problem bodies on the `/v1.0/*` behaviour. The
